...
export function KeyboardProvider({ children }: { children: ReactNode }) {
  ...
  const open = useCallback((s: KeyboardSession) => {
    setSession(s);
  }, []);
  ...
  useEffect(() => {
    if (!session) return;
    // Listener en fase de CAPTURA: corre antes que cualquier handler de React /
    // react-simple-keyboard. Necesario porque al apretar shift/123 la lib
    // re-renderiza el layout entero y los nodos viejos quedan fuera del árbol —
    // si esperáramos al bubble, `contains()` sobre el target original fallaría
    // y cerraríamos por error.
  ...
  // Scroll del input/elemento activo para que no quede tapado por el teclado.
  // Solo dispara cuando cambia la sesión (no en cada keystroke).
  ...
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      // Tras un frame, el teclado ya está en el DOM y podemos medir su altura.
      const kbEl = document.getElementById('virtual-keyboard-root');
      const kbHeight = kbEl?.getBoundingClientRect().height ?? 320;
      // Si el input ya está cómodamente arriba del teclado, no hacemos nada.
  ...
  return (
    <KeyboardCtx.Provider value={{ activeId: session?.id ?? null, open, close, update }}>
      {children}
...
function QwertyOverlay({ session, onClose }: { session: KeyboardSession; onClose: () => void }) {
  ...
  // Sincronizar el valor externo con el estado interno de la lib.
  useEffect(() => {
  ...
  const handleKeyPress = useCallback(
    (button: string) => {
      if (button === '{shift}' || button === '{shiftactive}') {
        ...
        // Auto-revertir shift después de tipear una letra (comportamiento estándar mobile)
        setLayoutName('default');
      }
    },
    [layoutName, onClose],
